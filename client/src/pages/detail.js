...
const typeKickers = { waterfall: 'Thác nước · Khu vực', cave: 'Hang động · Khu vực', historical: 'Di tích lịch sử · Khu vực', spiritual: 'Tâm linh · Khu vực' };
...
async function loadDestination() {
  ...
  try {
    ...
    // Map
    const detailMap = L.map('detailMap', { zoomControl: false, attributionControl: false, scrollWheelZoom: false }).setView([lat, lng], 14);
    ...
    geoFiles.forEach(({ file, color: c }) => {
      fetch(file).then(r => r.json()).then(d => {
        L.geoJSON(d, { style: { color: c, weight: 1.5, opacity: 0.5, fillColor: c, fillOpacity: 0.03 } }).addTo(detailMap);
      }).catch(() => {});
    });
...
function populateDest() {
  ...
  const name = escapeHtml(window.dest.name[lang] || window.dest.name.vi || window.dest.name.en) || '';
  const desc = window.dest.description ? (window.dest.description[lang] || window.dest.description.vi || window.dest.description.en || '') : '';
  const quoteStr = window.dest.quote ? (window.dest.quote[lang] || window.dest.quote.vi || window.dest.quote.en || '') : '';
  ...
  if (typeof I18nNew !== 'undefined') {
    document.getElementById('headingIntro').textContent = I18nNew.get('detail.intro') || I18nNew.getCommon('nav.about') || 'Giới thiệu';
  ...
  let notes = window.dest.visitor_notes || window.dest.metadata?.visitor_notes?.[lang] || window.dest.metadata?.visitor_notes?.vi;
  if (notes && Array.isArray(notes) && notes.length > 0) {
    notesBlock.style.display = 'block';
    notesList.innerHTML = notes.map(note => {
      if (typeof note === 'string') {
        let icon = '📌'; let text = note;
...
async function loadNearby(lat, lng, lang) {
  ...
  } catch (error) { console.error('Error loading nearby:', error); }
}
...
async function addComment() {
  ...
  try {
    ...
    else { alert('Lỗi: ' + (data.message || 'Không thể gửi bình luận')); }
...
function initDetailPage() {
  ...
  if (typeof I18nNew !== 'undefined') {
    I18nNew.init('detail').then(() => {
      document.addEventListener('i18n-changed', () => { if (window.dest) populateDest(); });
      loadDestination();
    });
  } else {
    loadDestination();
